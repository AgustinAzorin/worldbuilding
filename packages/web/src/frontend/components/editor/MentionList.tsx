  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden min-w-[200px] max-w-xs">
      {items.length > 0 ? (
            ...
            <li key={item.id} role="option" aria-selected={index === selectedIndex}>
              <button
                ...
                className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                  index === selectedIndex ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700 hover:bg-gray-50'
                }`}
        ...
        <p className="px-3 py-2 text-sm text-gray-400 italic">No se encontraron artículos</p>
      )}
    </div>
  )
})
